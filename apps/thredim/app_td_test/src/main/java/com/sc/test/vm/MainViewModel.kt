...
import com.sc.test.bean.AppInfoBean
import com.sc.test.utils.AssetsFileManager
import com.sc.test.utils.WavFileProcessor
...
class MainViewModel @Inject constructor(val spManager: SharedPreferencesManager) : BaseViewModel() {
    ...
    var wavFileProcessor: WavFileProcessor? = null
    var assetsFileManager: AssetsFileManager? = null

    fun initData() {

    }

    fun readAssetFile(context: Context) {
//        if (wavFileProcessor == null) wavFileProcessor = WavFileProcessor(context)
        if (assetsFileManager == null) assetsFileManager = AssetsFileManager(context)
        viewModelScope.launch(Dispatchers.IO) {
//            wavFileProcessor?.splitStereoWav("test.wav")
            assetsFileManager?.copyAssetsFolderToInternal()
        }
    }
