...
public class MinaUtil {
    ...
    public static Map<IoSession, IMySession> sessionMap = new HashMap<>();

    public static IMySession convertToISession(final IoSession ioSession, final AbstractSocket _socket) {
        IMySession session = sessionMap.get(ioSession);

        if (session == null) {
            session = new IMySession() {
                @Override
                public long getId() {
...
//                }
    ...
    static KeepAliveFilter initClientKeepAlive(Config config, final AbstractSocket socket) {
        ...
        }, IdleStatus.WRITER_IDLE, KeepAliveRequestTimeoutHandler.CLOSE, interval, timeout);
...
//                return keepAliveListener.getKeepAliveMessage(iSession, null);
            ...
            public Object getResponse(IoSession ioSession, Object o) {
                ISession iSession = MinaUtil.convertToISession(ioSession, socket);
                return keepAliveListener.getKeepAliveMessage(iSession, o);
            }
